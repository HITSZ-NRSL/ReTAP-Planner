...
void KD_TREE<PointType>::start_thread() {
  ...
  printf("<ikd-tree::start_thread>: Multi thread started \n");
}
...
void KD_TREE<PointType>::multi_thread_rebuild() {
  ...
  while (!terminated) {
    ...
    if (Rebuild_Ptr != nullptr) {
      ...
      // int valid_old =
      // old_root_node->TreeSize-old_root_node->invalid_point_num; // wgh:
      // unused variable. int valid_new =
      // new_root_node->TreeSize-new_root_node->invalid_point_num; // wgh:
      // unused variable.
  ...
  printf("<ikd-tree::multi_thread_rebuild>: Rebuild thread terminated normally\n");
}
...
void KD_TREE<PointType>::BuildTree(KD_TREE_NODE **root, int l, int r,
  ...
  *root = new KD_TREE_NODE;
  ...
  int div_axis = 0;
  int i;
  // Find the best division Axis (wgh
  // 也即分布最分散的那个轴，或者说最大值减最小值之差最大的那个轴)
...
// wgh 工具属性，更新指定节点的min/max-range, treesize, radius_sq,
// alpha_del等信息。
template <typename PointType>
